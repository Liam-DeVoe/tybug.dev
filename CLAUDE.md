...
- **`theme/static/css/`** — SCSS source files. `theme.scss` imports `_base.scss` (design-token variables), `_rouge.scss` (syntax highlighting), and `_figure.scss` (the figure/chart system); compiled to CSS at build time by the webassets plugin. The colour palette lives at the top of `_base.scss`.
